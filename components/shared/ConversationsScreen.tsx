// components/shared/ConversationsScreen.tsx
// Shared conversation list component for both Travelers and Guides
...
type UserRole = "Traveler" | "Guide" | "Creator" | "Driver" | "Admin";
...
const getRoleConfig = (userRole: UserRole) => {
    const configs: Record<UserRole, {
        chatDetailPath: string;
        newChatPath: string | null;
        showNewChatButton: boolean;
        emptyStateText: string;
        participantBadge: (role?: string) => string | null;
    }> = {
        Traveler: {
            chatDetailPath: "/(traveler)/(conversations)/[id]",
            newChatPath: "/(traveler)/(conversations)/new",
            showNewChatButton: true,
            emptyStateText: "Start a conversation with someone",
        ...
        Guide: {
            ...
            participantBadge: () => "Traveler",
        },
        Creator: {
            chatDetailPath: "/(creator)/(conversations)/[id]",
            newChatPath: null,
            showNewChatButton: false,
            emptyStateText: "Conversations with travelers will appear here when they book your experiences",
            participantBadge: () => "Traveler",
        },
        Driver: {
            chatDetailPath: "/(driver)/(conversations)/[id]",
            newChatPath: null,
            showNewChatButton: false,
            emptyStateText: "Conversations with travelers will appear here when you accept trip requests",
            participantBadge: () => "Traveler",
        ...
        Admin: {
            chatDetailPath: "/(admin)/(conversations)/[id]",
            newChatPath: "/(admin)/(conversations)/new",
            showNewChatButton: true,
            emptyStateText: "No conversations yet",
            participantBadge: (role) => role || null,
        },
    };

    return configs[userRole] || configs.Traveler;
};

// ============ MAIN COMPONENT ============
const ConversationsScreen = ({ userRole }: ConversationsScreenProps) => {
    ...
    useFocusEffect(
        useCallback(() => {
            console.log(`📱 ${userRole} Conversations screen focused`);
            ...
            return () => {
                console.log(`📱 ${userRole} Conversations screen unfocused`);
                socketService.offNewMessage();
            };
        }, [fetchConversations, handleNewMessage, userRole])
    );
    ...
    const handleConversationPress = async (conversation: Conversation) => {
        ...
        // Navigate to role-specific conversation detail screen
        router.push({
            pathname: roleConfig.chatDetailPath as any,
            params: {
    ...
    const handleNewChat = () => {
        if (roleConfig.newChatPath) {
            router.push(roleConfig.newChatPath as any);
        }
    };
    ...
    return (
        <SafeAreaView className="flex-1 bg-[#fff]">
                ...
                {filteredConversations.length === 0 ? (
                    ...
                    <View className="px-4">
                        {filteredConversations.map((conversation) => {
                            ...
                            const badgeText = roleConfig.participantBadge(otherUser?.role);
                                                ...
                                                <View className="flex-row items-center flex-1 mr-2">
                                                    ...
                                                    {badgeText && (
                                                        <View className="bg-blue-50 rounded-full px-2 py-0.5 ml-2">
                                                            <Text className="text-blue-600 font-onest text-xs">
                                                                {badgeText}
                                                            </Text>
                                                        </View>
                                                    )}
            ...
            {/* New Conversation FAB - Only shown for roles that can start chats */}
            {roleConfig.showNewChatButton && (
                <TouchableOpacity
