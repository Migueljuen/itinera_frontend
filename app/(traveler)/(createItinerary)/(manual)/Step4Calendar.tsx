...
const Step4Calendar: React.FC<StepProps> = ({
  ...
  const [daySelectionModalVisible, setDaySelectionModalVisible] =
    useState(false);
  const [selectedUnassignedExperience, setSelectedUnassignedExperience] =
    useState<UnassignedExperience | null>(null);
  ...
  useEffect(() => {
    const generateCalendarDays = () => {
      ...
      while (currentDate <= endDate) {
        ...
        const dayItems = (formData.items || []).filter(
          (item) =>
    ...
    const generateUnassignedExperiences = () => {
      ...
      const uniqueUnassigned = unassigned.filter(
        (experience, index, self) =>
          index ===
          self.findIndex((e) => e.experience_id === experience.experience_id)
      );
    ...
    const fetchExperiences = async () => {
      try {
        ...
        const uniqueExperienceIds = [
          ...new Set((formData.items || []).map((item) => item.experience_id)),
        ];

        const experiencePromises = uniqueExperienceIds.map(
          async (experienceId) => {
            try {
              // Fetch full experience data including title
              const experienceResponse = await fetch(
                `${API_URL}/experience/${experienceId}`
              );
              ...
              const availabilityResponse = await fetch(
                `${API_URL}/experience/availability/${experienceId}`
              );
              ...
              console.error(
                `Error fetching data for experience ${experienceId}:`,
                error
              );
  ...
  const handleRemoveUnassignedExperience = (
    experience: UnassignedExperience
  ) => {
    Alert.alert(
      "Remove Experience",
  ...
  const handleUnassignedExperienceClick = (
    experience: UnassignedExperience
  ) => {
  ...
  const handleDaySelection = async (day: CalendarDay) => {
    ...
    const experience = experiences.find(
      (exp) => exp.id === selectedUnassignedExperience.experience_id
    );
    if (!experience) {
    ...
    const dayAvailability = experience.availability.find(
      (avail) => avail.day_of_week === day.dayName
    );
  ...
  const handleAssignedItemClick = (item: ItineraryItem, day: CalendarDay) => {
    ...
    const otherDayItems = day.items.filter(
      (dayItem) => dayItem.experience_id !== item.experience_id
    );
  ...
  const handleTimeSlotSelect = (timeSlot: TimeSlot) => {
    if (selectedUnassignedExperience && selectedDay) {
      ...
      const updatedItems = (formData.items || []).map((item) => {
        if (
          item.experience_id === selectedItem.experience_id &&
      ...
      <View key={day.dayNumber} className="mb-4">
        ...
        <View
          className={`p-3 rounded-t-lg ${
            day.isToday ? "bg-primary" : "bg-gray-100"
          }`}
        >
          <Text
          ...
          <Text
            className={`font-onest-medium text-sm ${
                    ...
                    <View className="flex-1">
                      ...
                      <View className="flex-row items-center mt-1">
                        ...
                        <Text className="text-sm text-gray-600 ml-1 font-onest-medium">
                          {formatTime(item.start_time + ":00")} -{" "}
                          {formatTime(item.end_time + ":00")}
                        </Text>
                      </View>
                    </View>
                    <View className="flex-row items-center">
          ...
          {unassignedExperiences.length > 0 && (
            <View className="bg-yellow-50 p-3 rounded-lg mb-4 flex-row items-center">
              ...
              <Text className="text-sm text-yellow-700 ml-2 flex-1 font-onest">
                Tap on unscheduled experiences above to assign them to specific
                days
              </Text>
            </View>
          )}
          ...
          <View className="bg-white rounded-t-3xl h-[32rem]">
            ...
            <ScrollView className="flex-1">
              {calendarDays.map((day) => {
                ...
                const experience = experiences.find(
                  (exp) =>
                    exp.id === selectedUnassignedExperience?.experience_id
                );
                const hasAvailability =
                  experience?.availability.some(
                    (avail) => avail.day_of_week === day.dayName
                  ) || false;
                ...
                return (
                  <TouchableOpacity
                    ...
                    onPress={() =>
                      hasAvailability ? handleDaySelection(day) : null
                    }
                  ...
                  >
                    <View>
                      ...
                      <Text className="text-xs text-gray-500 font-onest mt-1">
                        {day.items.length} experience
                        {day.items.length !== 1 ? "s" : ""} scheduled
                      </Text>
                      {!hasAvailability && (
        ...
        <View className="flex-1 justify-end bg-black/50">
          <View className="bg-white rounded-t-3xl h-[32rem]">
            <View className="p-4 border-b border-gray-200">
              ...
              <Text className="text-sm text-gray-600 mt-1 font-onest">
                {selectedUnassignedExperience?.title ||
                  `Experience ${selectedItem?.experience_id}`}
              </Text>
              {selectedDay && (
        ...
        <View className="flex-row justify-between">
          ...
          <TouchableOpacity
            ...
            className={`py-4 px-8 rounded-xl ${
              isScheduleComplete() ? "bg-primary" : "bg-gray-300"
            }`}
            activeOpacity={0.7}
          ...
          >
            <Text
              className={`text-center font-onest-medium text-base ${
                isScheduleComplete() ? "text-white" : "text-gray-500"
              }`}
            >
              Finalize Itinerary
            </Text>
