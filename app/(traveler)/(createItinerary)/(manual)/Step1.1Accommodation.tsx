...
const Step1_1Accommodation: React.FC<StepProps> = ({
    ...
    const [checkOutDate, setCheckOutDate] = useState(accommodation.check_out || formData.end_date || '');
    ...
    const handleInputChange = (field: keyof Accommodation, value: string | number | undefined) => {
        setAccommodation(prev => ({
            ...prev,
            [field]: value
        }));
    ...
    const handleNext = () => {
        ...
        if (accommodation.name || accommodation.address) {
            if (!accommodation.name || !accommodation.address) {
                Alert.alert('Validation Error', 'Please fill in both accommodation name and address, or skip this step.');
                return;
            }
        }
        ...
        setFormData((prev: ItineraryFormData) => ({
            ...prev,
            accommodation: accommodation.name || accommodation.address ? accommodation : undefined
        }));
                        ...
                        <View className="text-center py-2">
                            ...
                            <View className="flex justify-evenly gap-6 border-t pt-8 border-gray-200">
                                ...
                                <View className="bg-white pb-4 mt-4 z-9">
                                    <Text className="font-onest-medium text-base mb-3">Stay Duration</Text>
                                    <TouchableOpacity
                                        ...
                                        activeOpacity={0.7}
                                    >
                                        <Text className={`text-base ${checkInDate ? 'text-black font-onest' : 'text-gray-500'}`}>
                                            {checkInDate && checkOutDate
                                                ? `${formatDisplayDate(checkInDate)} - ${formatDisplayDate(checkOutDate)}`
                                                : checkInDate
                                                    ? `${formatDisplayDate(checkInDate)} - Select check-out date`
                                    ...
                                    {checkInDate && !showCalendar && (
                                        <View className="flex-row justify-between mt-2">
                                            <View className="flex-1">
                                                <Text className="text-xs text-gray-500 font-onest">Check-in Date</Text>
                                                <Text className="text-sm font-onest-medium">{formatDisplayDate(checkInDate)}</Text>
                                            </View>
                                            <View className="flex-1">
                                                <Text className="text-xs text-gray-500 font-onest">Check-out Date</Text>
                                            ...
                                            {checkInDate && checkOutDate && (
                                                <View className="flex-1">
                                                    ...
                                                    <Text className="text-sm font-onest-medium">
                                                        {differenceInDays(new Date(checkOutDate), new Date(checkInDate)) + 1} days
                                                    </Text>
                                                </View>
                                            )}
                                        </View>
                                ...
                                <View className="mb-4">
                                    ...
                                    <View className="relative">
                                        <TextInput
                                            ...
                                            onChangeText={(text) => handleInputChange('booking_link', text)}
                                            keyboardType="url"
                                            autoCapitalize="none"
                                            placeholderTextColor="#9CA3AF"
                                        />
