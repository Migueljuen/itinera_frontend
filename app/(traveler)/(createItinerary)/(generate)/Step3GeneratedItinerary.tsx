...
const EnhancedErrorDisplay: React.FC<{
    ...
    city: string;
}> = ({ error, onTryAgain, onModifyPreferences, city }) => {
    return (
        <View className="flex-1 bg-gray-50 px-4 py-6">
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <View className="bg-white rounded-xl p-6 mb-4 border border-gray-200">
                    <View className="items-center mb-4">
                        ...
                        <Text className="font-onest text-sm text-gray-600 text-center mt-2">
                            We couldn't find any activities in {city} that match your current preferences
                        </Text>
                    </View>

                    {/* Filter Breakdown */}
                    {error.details?.filter_breakdown && (
                    ...
                    {error.details?.conflicting_preferences && error.details.conflicting_preferences.length > 0 && (
                        <View className="mb-4">
                            <Text className="font-onest-semibold text-sm text-red-700 mb-3">
                                ⚠️ Conflicting Preferences:
                            </Text>
                            {error.details.conflicting_preferences.map((conflict, index) => (
                                <View key={index} className="flex-row items-start mb-2">
                ...
                {/* Alternative Options */}
                {error.details?.alternative_options && (
                    <>
                        {/* Nearby Cities */}
                        {error.details.alternative_options.nearby_cities?.length > 0 && (
                            <View className="bg-white rounded-xl p-4 mb-4 border border-gray-200">
                                <Text className="font-onest-semibold text-base text-gray-900 mb-3">
                                    🏙️ Try Nearby Cities
                                </Text>
                                <View className="space-y-2">
                                    {error.details.alternative_options.nearby_cities.map((cityOption, index) => (
                        ...
                        {/* Popular Experiences */}
                        {error.details.alternative_options.popular_experiences?.length > 0 && (
                            <View className="bg-white rounded-xl p-4 mb-4 border border-gray-200">
                                <Text className="font-onest-semibold text-base text-gray-900 mb-3">
                                    🔥 Popular in {city}
                                </Text>
                                ...
                                <View className="space-y-3">
                                    {error.details.alternative_options.popular_experiences.slice(0, 3).map((exp, index) => (
...
const CollapsibleFilter: React.FC<{
    ...
    expandFilter?: () => void;
}> = ({ preferences, city, startDate, endDate, onRegenerateWithNewFilters, expandFilter }) => {
